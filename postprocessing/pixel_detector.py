...
def process_image(
    ...
    """
    Process an image to detect pixel art scaling and optionally reduce color palette.

    Args:
        input_path: Path to input image
        output_path: Path to save output image (default: "output.png")
        max_colors: Max colors for computation (default: 128)
        reduce_palette: Whether to automatically reduce the image to predicted color palette (default: False)

    Returns:
        tuple: (output_image, scale_factor, color_count) or None if file doesn't exist
    """
    ...
    print(
        f"Size detected and reduced from {image.width}x{image.height} to {downscale.width}x{downscale.height} in {round(time.time()*1000)-start} milliseconds"
    )
    ...
    if reduce_palette:
        ...
        print(
            f"Palette reduced to {best_k} colors in {round(time.time()*1000)-start} milliseconds"
        )

    output.save(output_path)
    # upscale the image to the original size with nearest neighbor
    output = output.resize(image.size, Image.NEAREST)
    output.save(output_path + "_" + str(max_colors) + "_upscaled.png")
    print(f"Upscaled image saved to {output_path}_upscaled.png")

    return output, scale, color_count


if __name__ == "__main__":
    input_image_path = "output_dq/pookie-pixel-lora-sdxl-1759431722-3.png"
    output_image_path = "output_postprocessing/pixelized.png"
    max_colors = 32
    reduce_palette = True
    process_image(input_image_path, output_image_path, max_colors, reduce_palette)
